---
title: FIA
slug: fia
description: ""
date: "2026-03-05T22:38:39.795362"
draft: false
sectors:
  - "public-sector"
  - sports
regions:
  - switzerland
years:
  - 2014
languages:
  - english
tags:
  - transport
industries:
  - "automotive-industry"
countries:
  - italy
revenue_buckets: []
operating_income_buckets: []
net_profit_buckets: []
lightings: []
perspectives: []
image_backgrounds: []
color_schemes: []
photography_genres: []
concepts: []
depths: []
image_effects: []
dominant_colors: []
color_tones: []
hair_styles: []
facial_expressions: []
clothing_styles: []
clothing_colors: []
posings: []
gazes: []
body_sections: []
logo_elements:
  - stacked
  - primary
  - symbol
  - horizontal
brand_colors:
  - blue
typographies: []
imagery_styles:
  - photography
  - professional
wikidata_description: Italian automotive company
has_twitter: false
has_guidelines: true
promotion_image_count: 0
guideline_count: 1
---
## Official Websites

- [https://www.fia.com/](https://www.fia.com/)

## Brand Guidelines

### 2014

**Logo Information:**

- The FIA Master Logo consists of a blue globe and motor cog symbol, representing motoring and Motor Sport associations. It is designed to show authority and unity, with uppercase letters 'F' and 'A', and a lowercase 'i'. A tagline 'A World in Motion' accompanies the logo, representing the mission of FIA.
- Positive and negative formats of the FIA Master Logo are shown. The blue positive logo should be used as the primary logo.
- The Logotype includes the text 'FEDERATION INTERNATIONALE DE L’AUTOMOBILE' in either stacked or horizontal arrangement. The preferred format is horizontal for best practice.

**Color Information:**

- The primary color for the FIA brand is blue. Additional information on the color palette to be provided later.

**Imagery Photography:**

- Photography is grouped into several categories, including Corporate Imagery, Motor Sport Imagery, Motoring Imagery, FIA Campaigns Imagery, and FIA Events Imagery. Specific guidelines may follow.

**Brand Voice:**

- FIA attributes include derivatives such as EXCELLENCE IN MOTION, IDEAS IN MOTION, ACTION IN MOTION, PASSION IN MOTION, SAFETY IN MOTION, HISTORY IN MOTION. These derivatives strengthen the FIA brand across collateral and campaigns.
- FIA's mission is to act as a professional and caring Global Federation driving innovation, passion, sustainability, safety, and governance in motoring and Motor Sport, and to be the Global Voice for a World in Motion.

**Notes:**

- The guidelines aim to ensure brand consistency across online, print, and event communications. Applications such as translations cannot be presented due to extensive use. Contact FIA for clarification.
- The contents page outlines various brand assets covered in the guidelines, such as FIA division logos, FIA flags, sub-brand logos, typography usage, and campaign guidelines.

## Additional Properties

| Property | Value |
|----------|-------|
| Founded By | Giovanni Agnelli |
| Country | Italy |
| Owned By | Stellantis Europe (from 2007),  (until 2007) |
| Headquarters Location | Turin |
| Freebase Id | /m/04n3_w4 |
| Topic'S Main Category | Category:Fiat |
| Inception | 1899-07-11 |
| Commons Gallery | Fiat |
| Encyclopædia Britannica Online Id | topic/Fiat-SpA |
| Location Of Formation | Turin |
| Legal Form | società per azioni |
| Industry | automotive industry |
| Logo Image | FIAT logo (2020).svg (from 2020, until 2022), Fiat Automobiles logo.svg (from 2006, until 2020), Fiat logo 1968.svg (from 1968, until 1999), FIAT logo coloured.svg (from 2022) |
| Quora Topic Id | Fiat-Automobiles, Fiat-3 |
| Itu/Iso/Iec Object Id | 1.3.6.1.4.1.21339 |
| Permid | 4295875588 |
| Chief Executive Officer | Olivier François (from 2011) |
| Parent Organization Or Unit | Fiat Chrysler Automobiles (from 2014, until 2021), Fiat Group (until 2014), Stellantis (from 2021) |
| Great Norwegian Encyclopedia Id | Fiat |
| Ne.Se Id | fiat |
| Dagens Nyheter Topic Id | fiat |
| Uk Parliament Thesaurus Id | 34199 |
| Youtube Channel Id | UC_nBvBIV0K6P5C3tA27Hwrg (as of 2020-12-07, from 2009-03-20) |
| Facebook Username | Fiat |
| X (Twitter) Username | fiat (as of 2020-04-19, from 2009-03-20), fiatontheweb (as of 2015-08-16, from 2009-03-20) |
| Wmi Code | ZFA |
| Gran Enciclopèdia Catalana Id (Former Scheme) | 0230067 |
| Microsoft Academic Id (Discontinued) | 125077922 |
| Product Or Material Produced | car |
| Osm Name Suggestion Index Id | fiat-13eeb7, fiat-dc8719 |
| Social Media Followers | 179,225 (as of 2021-01-04), 176,901 (as of 2020-04-19), 58,300 (as of 2021-07-03), 182,797 (as of 2022-01-08), 189,787 (as of 2023-02-07) (+3 more) |
| Open Library Subject Id | fiat_(firm) |
| Image | FiatMirafiori6.jpg |
| Lex Id | Fiat |
| Musicbrainz Label Id | 1842edaa-a1c0-4ff9-bd2b-fe6e65c8f77b |
| Nl Cr Aut Id | ph208168 |
| Golden Id | Fiat_Automobiles-9X95KE |
| Pixiv Encyclopedia Id | フィアット |
| Crunchbase Organization Id | fiat-63e6 |
| Encyclopedia Of China (Third Edition) Id | 462729 |
| Pm20 Film Section Id | h1/co/A0261H/0237/L (from 1931) |
| Topic Has Template | Template:Fiat |
| Great Russian Encyclopedia Portal Id | fiat-fe3a1e |
| Inapp Author Id | 10736 |
| Gran Enciclopèdia Catalana Id | fabbrica-italiana-automobili-torino |
| Museum Digital Id | 81284 |
| Vikidia Article Id | es:Fiat, fr:Fiat |
| Chairperson | John Elkann (from 2010) |
| Niconicopedia Id | フィアット |
| France 24 Topic Id (French) | fiat |
| Reddit Topic Id | fiat |
| ‎Private Enterprise Number | 21339 |
| Eu Vat Number | IT09487650013 |
| Official Name | Fiat Automobiles S.p.A. |
| Instagram Username | fiat, fiatontheweb |
| Google+ Id | 111811790542649932315 |
| Trustpilot Company Id | www.fiat.com |
| Linkedin Company Or Organization Id | fiatauto, fiat-brand |
| De Agostini Id | FIAT+S.p.A. |
| Treccani Id | fiat |
| Treccani'S Dizionario Di Economia E Finanza Id | fiat |
| Fandom Article Id | logos:Fiat |
| Category For Employees Of The Organization | Category:Fiat people |
