---
title: DFW
slug: dfw
description: ""
date: "2026-03-05T15:56:46.097141"
draft: false
sectors:
  - "public-transportation"
  - tourism
regions:
  - usa
years:
  - 2016
languages:
  - english
tags:
  - airport
industries: []
countries:
  - "united-states"
revenue_buckets: []
operating_income_buckets: []
net_profit_buckets: []
lightings: []
perspectives: []
image_backgrounds: []
color_schemes: []
photography_genres: []
concepts: []
depths: []
image_effects: []
dominant_colors: []
color_tones: []
hair_styles: []
facial_expressions: []
clothing_styles: []
clothing_colors: []
posings: []
gazes: []
body_sections: []
logo_elements:
  - secondary
  - primary
brand_colors:
  - purple
  - yellow
  - gray
  - orange
  - black
  - white
  - blue
typographies:
  - times
  - arial
imagery_styles:
  - monochrome
  - bold
  - graphic
  - photography
wikidata_description: "airport in Irving serving the Dallas-Fort Worth metro area in Texas, United States"
has_twitter: false
has_guidelines: true
promotion_image_count: 0
guideline_count: 1
---
## Official Websites

- [http://www.dfwairport.com](http://www.dfwairport.com)
- [https://www.dfwairport.com/](https://www.dfwairport.com/)

## Brand Guidelines

### 2016

**Logo Information:**

- The DFW logo represents key brand elements such as clarity and the idea of connection. The primary logo version uses DFW Orange and can be used on most touchpoints, especially inside the airport. Secondary logo versions add the full name, 'Dallas Fort Worth International Airport,' and are available in black and reversed versions.
- Clear space guidelines mandate a minimum of 1.75x the journey line's thickness. The DFW logo uses a proportional relationship to ensure clarity within its usage.
- The logo variations include a primary logo, secondary logo with additional context, and monochrome (black and white or reversed) options. Guidelines emphasize maintaining whitespace for cleanliness.
- Avoid incorrect usage of the journey line, which includes forming maze-like or complicated shapes, adding embellishments, letting it interact improperly with text or secondary elements, or using unsupported colors.

**Color Information:**

- Primary brand colors include DFW Orange, Light Gray, Dark Gray, Blue, Purple, and Yellow. Each color includes multiple system guidelines, including embroidery threads (Madeira), plastics (Pantone Q-codes), paint references (Matthews Paint), and vinyl specifications (Scotchcal).
- Color proportions: Most backgrounds should use white or DFW Light Gray. DFW Orange and Light Gray are the primary colors used for text and highlights. Secondary palette colors should be used sparingly. Tints of gray should be avoided, with recommended increments of 20% for other colors.

**Typography Information:**

- The primary typeface is Neue Haas Unica, selected for its warm and sophisticated look, with weights available in Light, Regular, Medium, and Bold (including italic styles). For accessibility and system compatibility, the Arial typeface is used as a secondary font in digital applications.

**Imagery Photography:**

- The journey line in graphic and photography elements illustrates the brand's core ideas, enhancing visual interest or drawing attention to key information. It must have a clear space for proper use and may weave around photography subjects.

**Spacing Layout:**

- The journey line's minimum clear space is 1.75 times its thickness. Its thickness should align with the channel of the DFW logo and its size must never fall below certain proportions for cohesiveness and legibility.

**Brand Voice:**

- The messaging tone of voice focuses on warmth, connection, optimism, advocacy, and trust. Central messaging themes include discovery, connection, and growth. The brand narrates experiential stories about journeys and future advocacy for travelers.

**Notes:**

- Important trademark notices: DFW, the DFW logo, and other marks are registered trademarks of the Dallas Fort Worth International Airport.
- All fonts and typefaces should ensure system compatibility before installation or purchase. The recommended paneuropean character map for extended linguistic support is included.

## Additional Properties

| Property | Value |
|----------|-------|
| Page Banner | WV banner DFW terminal D.jpg |
| Iata Airport Code | DFW |
| Icao Airport Code | KDFW |
| Faa Airport Code | DFW |
| Image | DFWAirportOverview.jpg |
| Country | United States |
| Located In The Administrative Territorial Entity | Texas |
| Owned By | Dallas, Fort Worth |
| Coordinate Location | 32.896944444444, -97.038055555556 |
| Freebase Id | /m/01kyg3 |
| Gnd Id | 4427706-4 |
| Structurae Structure Id | 10001259 |
| Viaf Cluster Id | 153670989 |
| Runway | 13L/31R, 13R/31L, 17C/35C, 17L/35R, 17R/35L (+2 more) |
| Elevation Above Sea Level | 607 foot |
| X (Twitter) Username | dfwairport (as of 2020-04-19, from 2009-04-23), DFWAirport |
| Place Served By Transport Hub | Dallas-Fort Worth metroplex, Mansfield |
| Named After | Dallas, Fort Worth, Dallas-Fort Worth metroplex |
| Date Of Official Opening | 1974-01-13 |
| Subreddit | DFWAirport (as of 2023-05-24, from 2014-12-25) |
| Gnis Feature Id | 1386500 |
| Encyclopædia Britannica Online Id | topic/Dallas-Fort-Worth-International-Airport |
| Google Maps Customer Id | 8300766736265584668 |
| Area | 6,963 hectare |
| Library Of Congress Authority Id | n85149792 |
| Interchange Station | DFW Airport North, DFW Airport/Terminal B, DFW Airport station |
| Topic'S Main Category | Category:Dallas/Fort Worth International Airport |
| Handbook Of Texas Id | epd01 |
| Facebook Username | DFWAirport |
| Instagram Username | dfwairport |
| Pinterest Username | dfwairport |
| Different From | Washington Dulles International Airport |
| Logo Image | DFW airport logo.svg |
| Social Media Followers | 69,491 (as of 2021-01-09), 68,125 (as of 2020-04-19), 79,080 (as of 2022-02-21), 92,216 (as of 2023-02-07) |
| State Of Use | in use |
| Dpla Subject Term | DFW Airport, Dallas Fort Worth International Airport |
| Isni | 0000000403975903, 000000010292511X |
| Wolfram Language Entity Code | Entity["Airport", "KDFW"] |
| Patronage | 7,091,159 (as of 1974-01-01), 7,293,265 (as of 1975-01-01), 7,986,004 (as of 1976-01-01), 8,594,004 (as of 1977-01-01), 13,015,249 (as of 1978-01-01) (+46 more) |
| Apple Maps Id | 11998645999503638677 |
| Yelp Id | dallas-fort-worth-international-airport-dfw-dallas-3 |
| Linkedin Company Or Organization Id | dfwairport |
| Official Map Url | https://www.dfwairport.com/map/ |
| App Store Developer Id | 587942748 |
| Google Play Developer Slug | Dallas/Fort Worth International Airport |
| Official Jobs Url | https://careers.dfwairport.com/ |
| Mapy.Com Id | osm&id=17500540 |
| Openstreetmap Relation Id | 1870401 |
| Babelnet Id | 01829747n |
| Natural Atlas Id | 2074110 |
| Bing Entity Id | 2b32c6c2-2325-a36d-6ea2-10b6fce09dc7 |
| Fast Id | 624622 |
| Un/Locode | USDFW |
| Wikimapia Id | 1486 |
| Short Name | Dallas-Fort Worth, Dallas-Fort Worth, Dallas/Fort Worth |
| Located In/On Physical Feature | Grapevine, Irving, Euless, Coppell |
| Bbc Things Id | 87814c1e-8183-434b-beed-d3a75f3c6462 |
| Aerodrome Reference Point | 32.89723, -97.03769 |
| Located In Time Zone | America/Chicago |
| Scheduled Service Destination | Vancouver International Airport, Toronto Pearson International Airport, Frankfurt Airport, Tokyo International Airport, Narita International Airport |
| ‎Yale Lux Id | place/02898d57-ecbf-4945-b908-4677616b4a0f |
| Threads Username | dfwairport |
| Kisti Id | K000361169 |
