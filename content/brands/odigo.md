---
title: Odigo
slug: odigo
description: ""
date: "2026-03-13T09:22:52.329346"
draft: false
sectors:
  - software
  - telecom
regions:
  - france
years:
  - 2022
  - 2021
languages:
  - english
tags:
  - corporate
industries: []
countries: []
revenue_buckets: []
operating_income_buckets: []
net_profit_buckets: []
employees_buckets: []
total_assets_buckets: []
total_equity_buckets: []
market_cap_buckets: []
products_or_materials_produced: []
products: []
headquarters_locations:
  - "herzliya-pituah"
subsidiaries: []
foundation_dates:
  - 1998
lightings: []
perspectives: []
image_backgrounds: []
color_schemes: []
photography_genres: []
concepts: []
depths: []
image_effects: []
dominant_colors: []
color_tones: []
hair_styles: []
facial_expressions: []
clothing_styles: []
clothing_colors: []
posings: []
gazes: []
body_sections: []
logo_elements: []
brand_colors:
  - pink
typographies: []
imagery_styles:
  - monochrome
wikidata_description: defunct social networking service
wikidata_url: "https://www.wikidata.org/wiki/Q1984171"
has_twitter: false
has_guidelines: true
promotion_image_count: 0
guideline_count: 2
sample_image_urls: []
visual_term_image_urls: {}
---
## Official Websites

- [http://www.odigo.com](http://www.odigo.com)
- [https://www.odigo.com/](https://www.odigo.com/)

## Brand Guidelines

### 2022

*Guidelines data not available*

### 2021

**Logo Information:**

- Logo variants include standard full-color, monochrome options (Indiogo text + 70% arrow tint), negatives, and Pink Soda color variants for use in dark backgrounds.

**Color Information:**

- Color 'Pink Soda' used primarily for arrows or text in specific uses like background emphasis.
- Color 'Pink Soda' used primarily for arrows or text in specific uses like background emphasis.

**Notes:**

- General advice related to adhering closely to defined guidelines for proportions and context-sensitive adjustments.
- General advice related to adhering closely to defined guidelines for proportions and context-sensitive adjustments.
- General advice related to adhering closely to defined guidelines for proportions and context-sensitive adjustments.

## Additional Properties

| Property | Value |
|----------|-------|
| Wikidata Link | [Open Wikidata](https://www.wikidata.org/wiki/Q1984171) |

| Property | Value |
|----------|-------|
| Headquarters Location | [Herzliya Pituah](/headquarters_locations/herzliya-pituah/) |
| Foundation Date | [1998](/foundation_dates/1998/) |
| Freebase Id | /m/05hbh1 |
| Headquarters Location | Herzliya Pituah |
| Quora Topic Id | Odigo |
| Developer | Comverse |
| Inception | 1998 |
| Wine Appdb Id | 1185 |
