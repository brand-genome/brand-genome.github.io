---
title: Transport for Wales
slug: "transport-for-wales"
description: ""
date: "2026-03-05T22:38:39.188665"
draft: false
sectors:
  - "public-sector"
  - "public-transportation"
regions:
  - "united-kingdom"
years:
  - 2019
languages:
  - english
  - welsh
tags:
  - featured
  - transport
industries: []
countries:
  - "united-kingdom"
revenue_buckets: []
operating_income_buckets: []
net_profit_buckets: []
lightings: []
perspectives: []
image_backgrounds: []
color_schemes: []
photography_genres: []
concepts: []
depths: []
image_effects: []
dominant_colors: []
color_tones: []
hair_styles: []
facial_expressions: []
clothing_styles: []
clothing_colors: []
posings: []
gazes: []
body_sections: []
logo_elements: []
brand_colors: []
typographies: []
imagery_styles: []
wikidata_description: "transportation organisation in Wales, United Kingdom"
has_twitter: false
has_guidelines: true
promotion_image_count: 0
guideline_count: 2
---
## Official Websites

- [http://www.comisiynyddygymraeg.cymru/](http://www.comisiynyddygymraeg.cymru/)
- [https://tfw.gov.wales/](https://tfw.gov.wales/)
- [https://tfw.wales/](https://tfw.wales/)

## Brand Guidelines

### 2019

*Guidelines data not available*

### 2019

*Guidelines data not available*

## Additional Properties

| Property | Value |
|----------|-------|
| Uk Parliament Thesaurus Id | 436025 |
| Image | Transport wales logo18.svg |
| Inception | 2018-10-14 |
| Country | United Kingdom |
| Whatdotheyknow Organisation Id | tfw |
| Google Knowledge Graph Id | /g/11gcngklcc |
| Child Organization Or Unit | Transport for Wales Rail |
| Owned By | Welsh Government |
| Located In The Administrative Territorial Entity | Wales |
| Headquarters Location | Pontypridd |
| Member Of | International Rail Transport Committee |
| Osm Name Suggestion Index Id | transportforwales-c6e0b4 |
| X (Twitter) Username | transport_wales |
| Youtube Handle | transport_wales |
| Instagram Username | transport_wales |
