---
title: Argentina
slug: argentina
description: ""
date: "2026-03-05T15:56:46.056313"
draft: false
sectors:
  - "public-sector"
  - tourism
regions:
  - argentina
years:
  - 2018
languages:
  - spanish
tags:
  - regional
industries: []
countries:
  - argentina
revenue_buckets: []
operating_income_buckets: []
net_profit_buckets: []
lightings: []
perspectives: []
image_backgrounds: []
color_schemes: []
photography_genres: []
concepts: []
depths: []
image_effects: []
dominant_colors: []
color_tones: []
hair_styles: []
facial_expressions: []
clothing_styles: []
clothing_colors: []
posings: []
gazes: []
body_sections: []
logo_elements:
  - vertical
  - horizontal
brand_colors:
  - blue
  - "pantone-294"
typographies:
  - impact
  - gotham
imagery_styles:
  - monochrome
  - realistic
  - photography
  - vibrant
  - bold
wikidata_description: "men's national association football team representing Argentina"
has_twitter: false
has_guidelines: true
promotion_image_count: 0
guideline_count: 1
---
## Official Websites

- [https://argentina.ar/es](https://argentina.ar/es)
- [https://argentina.gob.ar/comunicacion-y-prensa/contenidos-de-difusion/marca-pais](https://argentina.gob.ar/comunicacion-y-prensa/contenidos-de-difusion/marca-pais)
- [https://www.argentina.gob.ar/reglamento-de-la-marca-pais](https://www.argentina.gob.ar/reglamento-de-la-marca-pais)

## Brand Guidelines

### 2018

**Logo Information:**

- Logo-related content identified on page 2: Esta guía de uso conjuga las normativas de aplicación de la identidad visual de la marca país...
- Example of logo identifier usage from page 4: Identificador principal.
- Details about main logo configurations on page 5: Identificador principal.
- Guidelines on monochrome and incorrect logo uses on page 11: ATENCIÓN, Para evitar incurrir en errores se recomienda siempre aplicar la marca a partir de un documento digital original...
- Cobranding rules for logo placement on page 14: Identificador. Determined applicable cases when Argentina brand shares communications with others.
- Restrictions on vertical and horizontal cobranding placement on page 15: Identificador - Cobranding.
- Area of protection and compatibility for logos during cobranding on page 16: Versión plena / Versión lineal.

**Color Information:**

- Institutional 'Celeste Marca País Argentina': CMYK: C75 M15 Y0 K0, RGB: R0 G167 B225, HEX: #00A7E1, PMS Coated & Uncoated: 2995. Page 19.
- Special application for blue tones: Pantone 294 (C100 M85 Y30 K20). Specified on page 23.
- General note indicating the importance of color rules on page 18: Color rules and key identifiers noted.

**Typography Information:**

- Typography family 'Gotham Rounded' detailed on page 24. Described as fluid, fresh, and versatile.
- Typography size applications laid out: Titles require impact, large and bold; text differences must be visibly notable. Page 27.
- Ideal placements for typography specified: 5 usable positions including corners and center. Page 26.

**Imagery Photography:**

- Key photography concept described: Vibrant, magnetic, spontaneous, scenarios realistic. People featured heavily as protagonists. Page 29.
- Details on protagonist imagery, emphasizing cut images, contained framing, and technical depth. Page 30.
- Warm, human-focused photography mentioned—attributes of vibrancy reflected through interaction contexts. Page 32.

**Spacing Layout:**

- Minimum distances clearly outlined for layout structures; vertical and horizontal examples in detail. Page 39.
- Misuses documented for spacing including poor margins and base spaces for container elements. Incorrect examples noted. Page 41.

**Brand Voice:**

- Tone defined as inclusive and visually warm. Messaging revolves around projecting joy and national pride. Page indicated: 33.
- Error guidelines around dated branding impacts meant to ensure positivity and vibrant national projection. Page 36.

**Notes:**

- General note from page 2: Introduction and brand usage team details specified.
- Contact reference and team duties outlined on page 2, confirming mandatory brand team inquiries.
- Extraneous imagery violations noted include altered coloring and distortion effects placement warnings assigned. Page overall 10+.

## Additional Properties

| Property | Value |
|----------|-------|
| Topic'S Main Category | Category:Argentina national football team |
| Freebase Id | /m/02bh_v |
| Participant In | 2008 Summer Olympics medal table, 1982 FIFA World Cup, 1986 FIFA World Cup, 1978 FIFA World Cup, 2002 FIFA World Cup (+17 more) |
| Sport | association football |
| Country For Sport | Argentina |
| X (Twitter) Username | Argentina (as of 2018-05-10, from 2014-05-14) |
| Country | Argentina |
| Official Name | Selección de fútbol de Argentina |
| Facebook Username | AFASeleccionArgentina |
| Fifa Country Code | ARG |
| Quora Topic Id | Argentina-National-Football-Team |
| Head Coach | Lionel Scaloni (from 2018-08-02) |
| Team Captain | Lionel Messi |
| Home Venue | Mas Monumental Stadium |
| Statistical Leader | Lionel Messi |
| Ranking | 1 (as of 2015-10-01), 1 (as of 2015-08-06), 1 (as of 2015-09-03), 1 (as of 2015-07-09), 1 (as of 2016-06-02) (+275 more) |
| Competition Class | men's association football |
| Owned By | Argentine Football Association |
| Flag Image | Flag of Argentina.svg |
| History Of Topic | history of the Argentina national football team |
| Instagram Username | afaseleccion |
| Transfermarkt Team Id | 3437 |
| Member Of | CONMEBOL (from 1916), FIFA (from 1912) |
| Social Media Followers | 3,881,797 (as of 2021-01-04), 3,279,491 (as of 2018-05-10), 4,325,662 (as of 2022-02-26), 5,398,516 (as of 2023-02-06) |
| X Topic Id | 731226234080788481 |
| Nickname | La Albiceleste |
| Competition Won | Copa Centenario Revolución de Mayo, 1921 South American Championship, 1925 South American Championship, 1927 South American Championship, 1929 South American Championship (+18 more) |
| Opta Football Team Id | 632 |
| Footballdatabase.Eu Team Id | 455 |
| Kit Supplier | Adidas AG |
| Fandom Article Id | football:Argentina_national_football_team |
| Google Doodle | celebrating-the-2022-world-cup-champions-argentina |
| Baidu Tieba Name | 阿根廷 (as of 2023-03-05) |
| Great Russian Encyclopedia Portal Id | sbornaia-argentiny-po-futbolu-93718b |
| L’Équipe Football Team Id | 642 |
| Goalzz.Com Team Id | 338 |
| Vikidia Article Id | it:Nazionale_di_calcio_dell'Argentina, fr:Équipe_d'Argentine_de_football |
| Bbc Things Id | da7ff3fd-eb71-4cf8-abb1-a8d025e9e135 |
| National Football Teams.Com Team Id | 9 |
| Scorebar / Soccerway Team Id | 132 |
| Short Name | Аргентына, Argentina |
| Category For Members Of A Team | Category:Argentina men's international footballers |
| Hindustan Times Topic Id | argentina-football-team |
| Subclass Of | national sports team |
