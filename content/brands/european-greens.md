---
title: European Greens
slug: "european-greens"
description: ""
date: "2026-03-05T22:38:39.562519"
draft: false
sectors:
  - ngo
regions:
  - europe
years:
  - 2017
languages:
  - english
tags:
  - community
industries: []
countries: []
revenue_buckets: []
operating_income_buckets: []
net_profit_buckets: []
lightings: []
perspectives: []
image_backgrounds: []
color_schemes: []
photography_genres: []
concepts: []
depths: []
image_effects: []
dominant_colors: []
color_tones: []
hair_styles: []
facial_expressions: []
clothing_styles: []
clothing_colors: []
posings: []
gazes: []
body_sections: []
logo_elements:
  - primary
  - secondary
brand_colors:
  - white
  - "pantone-376"
  - green
typographies: []
imagery_styles: []
wikidata_description: "green-regionalist political group of the European Parliament"
has_twitter: false
has_guidelines: true
promotion_image_count: 0
guideline_count: 1
---
## Official Websites

- [http://www.theguardian.com/guardian-observer-style-guide-a](http://www.theguardian.com/guardian-observer-style-guide-a)
- [https://europeangreens.eu/](https://europeangreens.eu/)

## Brand Guidelines

### 2017

**Logo Information:**

- The logo incorporates the image of a sunflower with twelve petals representing the EU flag, symbolizing commitment to European values.
- The logo comprises three elements: logo mark, logo type, and mandatory logo shape.
- Isolation area rules: The logo is surrounded by a minimum area based on green space derived from a lowercase character size.
- Application rules for green background: The logo can disappear into the background only when used for specific contexts.
- Misuse rules: No shadow, reflection, rotation, or aggressive contrast allowed. Maintain white or green color.
- Logo variations include a full logo and a version with just the logo mark without text.

**Color Information:**

- Primary colors include Green (Pantone 376 C) and secondary colors are outlined but details not visible in extracted text.

**Typography Information:**

- Typography for headlines: Font family includes specific choices (unreadable details in text).

**Imagery Photography:**

- Imagery emphasizes open and responsible care; specific style details described in text format.

**Spacing Layout:**

- Spacing/layout grid ensuring logo alignment via isolation rules (see page 8).

**Brand Voice:**

- Brand voice aligns with openness, accountability, honesty, care, and responsibility. Emotionally positive with active tone.

**Notes:**

- Visual identity aims to unify online and offline communication for strong recognition, especially for elections.

## Additional Properties

| Property | Value |
|----------|-------|
| Topic'S Main Category | Category:Greens–European Free Alliance |
| Has Part(S) | European Green Party, European Free Alliance, European Pirate Party, Volt Europa |
| Freebase Id | /m/01_195 |
| Gnd Id | 10135530-0 |
| Chairperson | Philippe Lamberts (from 2014), Rebecca Harms (from 2009-01-01, until 2016), Heidi Hautala (from 1999, until 2004),  (from 1999, until 2002), Monica Frassoni (from 2004, until 2009) (+3 more) |
| X (Twitter) Username | GreensEFA (as of 2020-03-07, from 2010-08-27) |
| Facebook Username | greensefa |
| Follows | The Green Group in the European Parliament, European Radical Alliance |
| Inception | 1999 |
| Number Of Seats In Assembly | 71 (from 2019, until 2024), 53 |
| Youtube Channel Id | UC9emZ4AlBYk7tp5zeYfjRfg (as of 2020-12-10, from 2007-12-10) |
| Political Ideology | green politics, regionalism, minority rights |
| Google+ Id | 109647937437924753169 |
| Srgb Color Hex Triplet | 009900 |
| Official Name | Parliamentary Group of The Greens/European Free Alliance in the European Parliament |
| Short Name | Greens/EFA, Verts/ALE, Vihreät/EVA |
| Instagram Username | greensefa |
| Viaf Cluster Id | 133523863 |
| Distribution Map | The Greens–European Free Alliance MEPs map.svg |
| Flickr User Id | 21733269@N06 |
| Social Media Followers | 71,577 (as of 2021-01-06), 5,270 (as of 2021-09-24), 77,293 (as of 2022-02-21), 81,155 (as of 2023-02-12), 6,020 (as of 2023-02-17) (+6 more) |
| Uk Parliament Thesaurus Id | 37117 |
| Lex Id | Gruppen_De_Grønne/Den_Europæiske_Fri_Alliance |
| Mastodon Address | EP_GreensEFA@social.network.europa.eu (from 2022-12-16), GreensEP@respublicae.eu (from 2022-04-03) |
| Great Norwegian Encyclopedia Id | The_Greens–European_Free_Alliance |
| Logo Image | GreensEFA logo EN.svg (until 2020-02), GreensEFA logo FR.svg (until 2020-02), GreensEFA logo DE.svg (until 2020-02), GreensEFA logo-bg.svg (from 2020-02), GreensEFA logo-ca.svg (from 2020-02) (+28 more) |
| Small Logo Or Icon | GreensEFA.png |
| Library Of Congress Authority Id | no2006091153 |
| Eu Corporate Body Code | EP_GROUP_GREEN_EFA |
| Bluesky Handle | greens-efa.eu |
| Ne.Se Id | de-grönaeuropeiska-fria-alliansen |
