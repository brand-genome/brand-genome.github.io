---
title: "Barnes & Noble"
slug: "barnes-noble"
description: ""
date: "2026-03-05T15:56:46.060997"
draft: false
sectors:
  - "retail-commerce"
  - publishing
regions:
  - usa
years:
  - 2012
languages:
  - english
tags:
  - entertainment
  - cultural
industries:
  - bookselling
countries:
  - "united-states"
revenue_buckets:
  - "1b-10b"
operating_income_buckets: []
net_profit_buckets:
  - "10m-100m"
lightings: []
perspectives: []
image_backgrounds: []
color_schemes: []
photography_genres: []
concepts: []
depths: []
image_effects: []
dominant_colors: []
color_tones: []
hair_styles: []
facial_expressions: []
clothing_styles: []
clothing_colors: []
posings: []
gazes: []
body_sections: []
logo_elements:
  - secondary
  - primary
brand_colors:
  - black
  - white
  - red
  - green
typographies:
  - serif
  - display
imagery_styles:
  - minimal
  - bold
  - photography
wikidata_description: American book retailer
has_twitter: false
has_guidelines: true
promotion_image_count: 0
guideline_count: 1
---
## Official Websites

- [https://nook.barnesandnoble.com/](https://nook.barnesandnoble.com/)
- [https://www.barnesandnoble.com/](https://www.barnesandnoble.com/)

## Brand Guidelines

### 2012

**Logo Information:**

- Primary logo uses color genres such as full color, single color (PMS 632), grayscale (100% black, 55% black), and one-color black.
- Usage of the secondary logo in full color, single color, black, and white genres.
- Logo can be set on varying backgrounds using single color PMS 632, black and white, or reversed white (PMS 5455) depending on visibility needs.

**Color Information:**

- Brand colors include PMS 632 (HEX #00ffad), PMS 1817 (HEX #620007), PMS 179 (HEX #ff3303).
- Secondary colors include pairs such as PMS 179 (Fiery Red) and PMS 1817 Shadow; PMS 390 (Crisp Green) and PMS 7490 Shadow.
- Sub-brand colors: NOOK uses PMS 179 (HEX #ff3303), B&Ncollege uses PMS 7547 (HEX #0f1a21), pubit! uses PMS 390 (HEX #b0e814).

**Typography Information:**

- Barnes & Noble Heading: Bold typeface used for all caps headings in print, posters, billboards, and web applications.
- Barnes & Noble Regular: Thin typeface for larger bodies of text, usable in all caps or upper and lowercase.
- Barnes & Noble Serif: Italic typeface used for captions and small text to convey class and softness.

**Imagery Photography:**

- Photography style emphasizes airiness and welcoming atmosphere with brighter exposure and minimal contrast.
- Images should display reading, music, food, and store activities, making viewers feel part of the photograph.

**Spacing Layout:**

- Tagline clear space guideline: .5x spacing around the logo elements, where x represents the height of the tagline font size.

**Brand Voice:**

- Tagline: 'More Than Just Books' emphasizing the environment and home-like feeling Barnes & Noble offers to customers.

**Notes:**

- Logo guidelines stress visibility and legibility, recommending reversal on dark backgrounds using PMS 5455 for better contrast.

## Additional Properties

| Property | Value |
|----------|-------|
| Stock Exchange | New York Stock Exchange |
| Logo Image | Barnes and Noble logo.svg |
| Topic'S Main Category | Category:Barnes & Noble |
| Headquarters Location | New York City |
| Image | Barnes & Noble Fifth Ave flagship.jpg |
| Viaf Cluster Id | 133686263 |
| Country | United States |
| Inception | 1873, 1917 |
| Founded By | Leonard Riggio, William Robbins Barnes, Gilbert Clifford Noble |
| Location Of Formation | New York City |
| Industry | bookselling |
| Legal Entity Identifier | 549300WEQE1M8II5OS20 |
| Freebase Id | /m/01b7dt |
| Employees | 40,000 (as of 2018), 26,000 (as of 2017) |
| Quora Topic Id | Barnes-Noble-company |
| Permid | 4295903538 |
| Open Funder Registry Funder Id | 100004701 |
| Mac Address Block Large Id | 58671A, 64C667 |
| X (Twitter) Username | BNBuzz (as of 2020-03-20, from 2009-04-16) |
| Grid Id | grid.467243.0 |
| Product Or Material Produced | software |
| Isni | 0000000091349005 |
| Isin | US0677741094 |
| Owner Of | GameStop, SparkNotes |
| Library Of Congress Authority Id | n80036726 |
| Facebook Username | barnesandnoble |
| Instagram Username | barnesandnoble |
| Pinterest Username | barnesandnoble |
| Ror Id | 019w40127 |
| Boardgamegeek Game Publisher Id | 4922 |
| Littlesis Organization Id | 448 |
| Osm Name Suggestion Index Id | barnesandnoble-1bd7b6 |
| Described At Url | https://www.fundinguniverse.com/company-histories/barnes-noble-inc-history/ |
| Social Media Followers | 256,691 (as of 2021-01-05), 270,370 (as of 2022-01-27), 301,066 (as of 2023-02-12) |
| Crunchbase Organization Id | nook-media, barnesandnoble-com |
| Comparably Company Id | barnes-noble |
| Encyclopedia Of China (Third Edition) Id | 566692 |
| Central Index Key | 0000890491 |
| Google Knowledge Graph Id | /g/11cm13l5ql |
| Legal Form | Delaware corporation |
| Location Information Url | https://www.store-locator.barnesandnoble.com, https://stores.barnesandnoble.com/ |
| Reddit Topic Id | barnes_noble |
| ‎Yale Lux Id | group/bbdd247a-fe9d-4702-8ddc-31f72d9209ae |
| Subreddit | Barnesandnoble |
| Wikirate Company Id | 2266068 |
| Usb Vendor Id | 2080 |
| Location Url Match Pattern | ^https:\/\/stores\.barnesandnoble\.com\/store\(/d+)$ |
| Threads Username | barnesandnoble |

### Revenue History

- $5.20B (5,200,000,000 United States dollar)

### Net Profit History

- $75.90M (75,900,000 United States dollar)
