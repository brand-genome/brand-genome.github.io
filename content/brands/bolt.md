---
title: Bolt
slug: bolt
description: Identity designed by Arthur Foliard at Koto.
date: "2026-03-05T15:56:45.656382"
draft: false
sectors:
  - finance
  - software
regions:
  - usa
years:
  - 2025
  - 2024
  - 2019
languages:
  - english
tags:
  - financial
  - featured
  - internet
industries:
  - "peer-to-peer-ridesharing"
  - "information-technology"
  - "food-delivery-service"
countries:
  - estonia
revenue_buckets:
  - "1b-10b"
operating_income_buckets: []
net_profit_buckets:
  - "10m-100m"
lightings: []
perspectives: []
image_backgrounds: []
color_schemes: []
photography_genres: []
concepts: []
depths: []
image_effects: []
dominant_colors: []
color_tones: []
hair_styles: []
facial_expressions: []
clothing_styles: []
clothing_colors: []
posings: []
gazes: []
body_sections: []
logo_elements: []
brand_colors:
  - blue
  - black
  - white
typographies: []
imagery_styles: []
wikidata_description: "Estonian company in field of peer-to-peer ridesharing and food delivery"
has_twitter: false
has_guidelines: true
promotion_image_count: 0
guideline_count: 3
---
## Official Websites

- [http://www.bolt.com](http://www.bolt.com)
- [https://bolt.com/](https://bolt.com/)
- [https://help.bolt.com/](https://help.bolt.com/)

## Brand Guidelines

### 2025

*Guidelines data not available*

### 2024

*Guidelines data not available*

### 2019

**Logo Information:**

- Sufficient clear space should surround the logo to ensure legibility.
- When pairing the Bolt logo with others for partnerships or collaborations, align logos vertically and maintain one Bolt logo spacing between them.
- Guidelines for incorrect logo usage, such as no drop shadows, complex imagery, gradients, taglines, distortions, or unapproved color combinations.
- Production-ready logo files are available in the specified team drive. Usage should consider the correct format for print vs. digital and light vs. dark background compatibility.

**Color Information:**

- Core color palette includes Off Black, White, and Bolt Blue. Specify proportions like 50:50:25 and imply dark mode as the right mood.
- Color codes: (1) Off Black: HEX #181A20, RGB (24, 26, 32), CMYK (3, 2, 0, 87), PMS Neutral Black C (2) White: HEX #FFFFFF, RGB (255, 255, 255), CMYK (0, 0, 0, 0), PMS Opaque White (3) Bolt Blue: HEX #006CFF, RGB (0, 108, 255), CMYK (100, 58, 0, 0), PMS 285C

**Spacing Layout:**

- Ensure one Bolt logo mark spacing between logos in pairings.

**Brand Voice:**

- Taglines provided include: 'Building the future of ecommerce', 'Perfecting checkout', and 'The world's most advanced commerce engine.'

**Notes:**

- Contact IT for logo file access (Team Drives > Design > External).

## Additional Properties

| Property | Value |
|----------|-------|
| Industry | peer-to-peer ridesharing, food delivery service, information technology |
| Inception | 2013-08-03 |
| Headquarters Location | Tallinn |
| Logo Image | Bolt Technology logo.svg |
| Business Registry Code (Estonia) | 12417834 |
| Legal Form | private limited company |
| Country | Estonia |
| X (Twitter) Username | boltapp (as of 2020-04-19, from 2013-03-16) |
| Facebook Username | bolt |
| Instagram Username | bolt |
| Founded By | Markus Villig |
| Product Or Material Produced | Bolt Food, Bolt |
| Google Knowledge Graph Id | /g/1yprtz41d |
| Google Play Developer Id | 8420210619522248974 |
| App Store Developer Id | 675033633 |
| Social Media Followers | 17,353 (as of 2021-01-07), 13,326 (as of 2020-04-19), 26,669 (as of 2022-02-26), 36,764 (as of 2023-02-07) |
| Crunchbase Organization Id | mtakso |
| Investor | Fidelity Investments, Sequoia Capital, International Finance Corporation, D1 Capital Partners, European Investment Bank (+2 more) |
| Eu Vat Number | EE101721521 |
| Official Name | Bolt Technology OÜ |
| Eu Transparency Register Id | 995377734447-25 |
| Member Of | OpenStreetMap Foundation (from 2022-05) |
| Board Member | Markus Villig |
| Owned By | Mordor Management (as of 2024-03-23), D1 Master Holdco I (as of 2024-03-23), Mordor Management (as of 2025-02-15), Mercedes-Benz Mobility Services (as of 2025-02-15) |
| Trading Name | Bolt |
| Tiktok Username | Bolt |
| Linkedin Company Or Organization Id | bolt-eu |
| Employees | 4,104 (as of 2023) |
| Location Of Formation | Tallinn |
| Uk Parliament Thesaurus Id | 569725 |

### Revenue History

- $1.70B (1,704,000,000 euro (as of 2023))

### Net Profit History

- $92.00M (-92,000,000 euro (as of 2023))
