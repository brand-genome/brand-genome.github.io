---
title: Ministry of Digital Affairs
slug: "ministry-of-digital-affairs"
description: ""
date: "2026-03-05T22:38:39.302060"
draft: false
sectors:
  - "public-sector"
regions:
  - taiwan
years:
  - 2022
languages:
  - chinese
tags:
  - institution
industries: []
countries:
  - poland
revenue_buckets: []
operating_income_buckets: []
net_profit_buckets: []
lightings: []
perspectives: []
image_backgrounds: []
color_schemes: []
photography_genres: []
concepts: []
depths: []
image_effects: []
dominant_colors: []
color_tones: []
hair_styles: []
facial_expressions: []
clothing_styles: []
clothing_colors: []
posings: []
gazes: []
body_sections: []
logo_elements:
  - primary
brand_colors:
  - white
  - "pantone-430"
  - "pantone-101"
typographies:
  - poppins
imagery_styles: []
wikidata_description: "Polish ministry (2015-2020, 2023-)"
has_twitter: false
has_guidelines: true
promotion_image_count: 0
guideline_count: 1
---
## Official Websites

- [https://moda.gov.tw/en/](https://moda.gov.tw/en/)

## Brand Guidelines

### 2022

**Logo Information:**

- Various logo arrangements described including logo_solo (figure only in English/Chinese) and logo_threesome (figure + English + Chinese)
- Minimum clear zone for logo specified as 1.5x distance from other elements
- Minimum size requirement stated as no less than 7mm in height for physical objects.

**Color Information:**

- Principal color: Pantone 101, HEX #F4EA5A, RGB R244 G234 B90, CMYK C11 Y77.
- Support color: Pantone 430, HEX #7B868C, RGB R123 G134 B140, CMYK C60 M45 Y40.
- Material color white specified for use.

**Typography Information:**

- Primary font family: Poppins. Styles include Light, Regular, and Medium.
- Typography examples provided in English with varying weights for body text.

**Spacing Layout:**

- Defined minimum clear zone for logos (1.5x distance from neighboring elements).
- Minimum height requirement for physical applications set at 7mm.

**Notes:**

- Application examples include business cards, badges, file seals, and other branded materials.

## Additional Properties

| Property | Value |
|----------|-------|
| Country | Poland |
| Inception | 2015-12-08 |
| Viaf Cluster Id | 87146936699013781348 |
| Logo Image | Logo Ministerstwa Cyfryzacji.svg |
| Image | Ministerstwo Cyfryzacji 2015.jpg |
| Replaces | Ministry of Administration and Digitization (Poland) |
| Headquarters Location | Warsaw |
| Field Of Work | net politics |
| Chairperson | Marek Zagórski |
| X (Twitter) Username | mc_gov_pl (as of 2020-06-16, from 2012-02-28), CYFRA_GOV_PL |
| Instagram Username | mc_gov_pl, cyfra_gov_pl |
| Youtube Channel Id | UCDDeiw-RUfbe_aW12lI12eQ (as of 2020-12-11, from 2013-11-27) |
| Facebook Username | MinisterstwoCyfryzacji |
| Google Knowledge Graph Id | /g/11bxb8m3m1 |
| Part Of | Council of Ministers |
| Social Media Followers | 46,219 (as of 2021-01-07), 40,866 (as of 2020-06-16), 47,305 (as of 2022-02-28), 5,000 (as of 2022-03-02), 48,757 (as of 2023-02-04) (+6 more) |
| Product Or Material Produced | PESEL number |
| Item Operated | PESEL registry |
| Applies To Jurisdiction | Poland |
| Linkedin Company Or Organization Id | ministerstwo-cyfryzacji |
| Ror Id | 03q81ka58 |
