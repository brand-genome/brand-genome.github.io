---
title: Tea
slug: tea
description: ""
date: "2026-03-05T15:56:46.146759"
draft: false
sectors:
  - restaurant
  - drink
regions:
  - "united-kingdom"
years:
  - 2006
languages:
  - english
tags:
  - "food-beverage"
  - featured
industries: []
countries:
  - italy
revenue_buckets: []
operating_income_buckets: []
net_profit_buckets: []
lightings: []
perspectives: []
image_backgrounds: []
color_schemes: []
photography_genres: []
concepts: []
depths: []
image_effects: []
dominant_colors: []
color_tones: []
hair_styles: []
facial_expressions: []
clothing_styles: []
clothing_colors: []
posings: []
gazes: []
body_sections: []
logo_elements:
  - secondary
  - primary
brand_colors: []
typographies:
  - helvetica
imagery_styles:
  - bold
  - product
  - photography
wikidata_description: "opera house in Naples, Italy"
has_twitter: false
has_guidelines: true
promotion_image_count: 0
guideline_count: 1
---
## Brand Guidelines

### 2006

**Logo Information:**

- The PDF extensively covers logos including various forms such as positive, negative, circular, square, alternative 't' logo versions, and various color-mode formats like CMYK, RGB, and EPS files.

**Color Information:**

- Color palette details include Pantone, CMYK, RGB codes with specifications for primary and secondary color palettes.

**Typography Information:**

- Guidelines for typography include Helvetica Neue Bold and Light, specifying headline and body typefaces, number formatting, and guidelines for font sizes.

**Imagery Photography:**

- Imagery styles emphasize elements such as product photography, leaf characters, web icons, patterns, and historic images. Guidelines for appropriate graphical content placement are included.

**Spacing Layout:**

- Spacing and layout rules consist of clear zones around the logo, alignment guides, grid principles, and margin specifications.

**Brand Voice:**

- The brand voice rules include ensuring a consistent tone across messaging, focusing on making tea accessible, values surrounding quality, and clear communication principles.

**Notes:**

- The appendix contains references for complete logo files, formats, and printing advice, including Pantone and CMYK use. Additionally, it contains extra specifications for shop signage, packaging design, and menu presentation.

## Additional Properties

| Property | Value |
|----------|-------|
| Coordinate Location | 40.8373882298, 14.2496871007 |
| Architect | Giovanni Antonio Medrano, Angelo Carasale |
| Freebase Id | /m/031p43 |
| Musicbrainz Place Id | 2752fd71-bac4-4038-abd4-271bb55172d6 |
| Image | Teatro de San Carlos, Nápoles, Italia, 2023-03-25, DD 184-186 HDR.jpg |
| Country | Italy |
| Viaf Cluster Id | 126846559 |
| Gran Enciclopèdia Catalana Id (Former Scheme) | 0058570 |
| Located In The Administrative Territorial Entity | Naples |
| Great Russian Encyclopedia Online Id (2017) | 3532089 |
| Euta Theatre Id | 986 |
| Google Arts & Culture Partner Id | teatro-di-san-carlo |
| Heritage Designation | Italian national heritage |
| Phone Number | +39-081-797-2111 |
| Email Address | mailto:lettere@teatrosancarlo.it |
| Postal Code | 80132 |
| Bibliothèque Nationale De France Id | 120852376 |
| Idref Id | 029170508 |
| Carthalia Id | 2378 |
| Dbunico Mibact Id | 108225 |
| Isni | 0000000121093021 |
| Street Address | Via San Carlo 98/F - Napoli |
| Native Label | Real Teatro di San Carlo |
| Library Of Congress Authority Id | n83215452 |
| Category For The Interior Of The Item | Category:Interior of Teatro San Carlo (Naples) |
| Gran Enciclopèdia De La Música Id | 3767 |
| Topic'S Main Category | Category:Ballet of Teatro di San Carlo |
| Alexander Turnbull Library Id | 128426 |
| Share Catalogue Author Id | 183925 |
| Spap Id | a/d3dc8d6a-9f9c-4102-9e59-390a485dca93 |
| National Library Of Israel J9U Id | 987007268835505171 |
| Image Of Interior | Teatro San Carlo - View of stage from Royal Box.JPG, Teatro San Carlo large view.jpg |
| Inception | 1737-11-04 |
| Nl Cr Aut Id | kv20191048184 |
| Field Of Work | opera house, opera, concert, play |
| Fondoambiente.It Id | teatro-di-san-carlo |
| Gnd Id | 1190671050 |
| Great Russian Encyclopedia Portal Id | san-karlo-19ce1d |
| Gran Enciclopèdia Catalana Id | teatro-san-carlo |
| Named After | Charles Borromeo |
| ‎Yale Lux Id | place/0192e506-b7c8-4b9b-b50c-46ca19c671ad |
| Nacsis Cat Author Id | DA14548733 |
