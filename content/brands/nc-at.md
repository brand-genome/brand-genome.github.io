---
title: "N.C. A&T"
slug: "nc-at"
description: ""
date: "2026-03-05T15:56:45.932643"
draft: false
sectors:
  - university
  - education
regions:
  - usa
years:
  - 2019
languages:
  - english
tags:
  - education
  - university
industries: []
countries:
  - "united-states"
revenue_buckets: []
operating_income_buckets: []
net_profit_buckets: []
lightings: []
perspectives: []
image_backgrounds: []
color_schemes: []
photography_genres: []
concepts: []
depths: []
image_effects: []
dominant_colors: []
color_tones: []
hair_styles: []
facial_expressions: []
clothing_styles: []
clothing_colors: []
posings: []
gazes: []
body_sections: []
logo_elements:
  - secondary
  - primary
  - icon
brand_colors:
  - blue
  - gold
  - black
  - gray
typographies:
  - montserrat
imagery_styles: []
wikidata_description: university
has_twitter: false
has_guidelines: true
promotion_image_count: 0
guideline_count: 1
---
## Official Websites

- [https://online.ncat.edu/](https://online.ncat.edu/)
- [https://www.ncat.edu/](https://www.ncat.edu/)

## Brand Guidelines

### 2019

**Logo Information:**

- The university logos include primary, secondary, and icon versions. These are trademarks of North Carolina Agricultural and Technical State University to establish a uniform visual identity. Usage of these logos is restricted to recognized university entities; personal use of the logos is prohibited.
- College-level logos have three levels of usage: (1) Full college name for formal applications like business cards or letterheads, (2) Contextual college logos for viewbooks or external communication, (3) Simplified logos for applications already within the university context, such as signage.
- Clear space must be maintained around all logos to distinguish them from adjacent text or images. Details for measurements are not specified.

**Color Information:**

- Primary colors are 'Aggie Blue' (PMS 288, CMYK: 100, 65, 0, 31, RGB: 0, 70, 132, HEX: #004684) and 'Aggie Gold' (PMS 123, CMYK: 0, 30, 94, 0, RGB: 253, 185, 39, HEX: #FDB927).
- Accent color is Gray (PMS 7536, CMYK: 0, 0, 0, 30, RGB: 194, 194, 194, HEX: #C2C2C2). It is intended for supporting primary colors and adding hierarchy but must not dominate the design.

**Typography Information:**

- Primary font: Montserrat. Montserrat Black is for headlines or emphasized titles (uppercase only), and Montserrat Regular is for body copy.
- Secondary font: Alfon, used for secondary headlines in college-level applications and accent purposes such as quotes.

**Spacing Layout:**

- Logo spacing rules require a clear area around the logo, but specific clear space dimensions are not provided.

**Notes:**

- Logo usage notes emphasize that trademarks are protected and restricted to university entities.
- Typography note: Montserrat is highlighted as the main typeface with specific roles for its weights and all-uppercase usage in Montserrat Black.

## Additional Properties

| Property | Value |
|----------|-------|
| Country | United States |
| Topic'S Main Category | Category:North Carolina A&T State University |
| Member Of | Oak Ridge Associated Universities, ORCID, Inc. (as of 2023-10), Association of American Colleges and Universities, American Council on Education, National Humanities Alliance (+4 more) |
| Freebase Id | /m/02ldkf |
| Integrated Postsecondary Education Data System Id | 199102 |
| Inception | 1891 |
| Postal Code | 27411 |
| Parent Organization Or Unit | University of North Carolina |
| Headquarters Location | Greensboro |
| Carnegie Classification Of Institutions Of Higher Education | doctoral university: higher research activity, high undergraduate, research doctoral: STEM-dominant, four-year, medium, primarily residential, four-year, full-time, inclusive, higher transfer-in (+1 more) |
| Isni | 0000000102874439, 000000041936818X |
| Quora Topic Id | North-Carolina-Agricultural-and-Technical-State-University |
| Open Funder Registry Funder Id | 100008521 |
| Encyclopædia Britannica Online Id | place/North-Carolina-Agricultural-and-Technical-State-University |
| Category For Alumni Of Educational Institution | Category:North Carolina A&T State University alumni |
| Grid Id | grid.261037.1 |
| Category For Employees Of The Organization | Category:North Carolina A&T State University faculty |
| Logo Image | North Carolina A&T State University wordmark.svg |
| Microsoft Academic Id (Discontinued) | 35777872 |
| Street Address | 1601 E Market  St, Greensboro, NC, 27411 |
| Ringgold Id | 3616 |
| Library Of Congress Authority Id | n79015702 |
| Located In The Administrative Territorial Entity | Greensboro |
| X (Twitter) Username | ncatsuaggies (as of 2020-04-22, from 2009-06-30) |
| Facebook Username | ncatsuaggies |
| Instagram Username | ncatsuaggies |
| Ror Id | 02aze4h65 |
| On Focus List Of Wikimedia Project | WikiProject African diaspora |
| Viaf Cluster Id | 153589980 |
| Coordinate Location | 36.075352, -79.773628 |
| Child Organization Or Unit | F. D. Bluford Library |
| Content Partnership Category | Category:Media contributed by North Carolina Agricultural and Technical State University |
| Social Media Followers | 58,329 (as of 2021-01-03), 55,072 (as of 2020-04-22), 64,530 (as of 2022-02-20), 71,411 (as of 2023-02-09) |
| Baglama Gid | 678 |
| X Topic Id | 1328770387237236736 |
| Described By Source | Open GLAM Survey (as of 2021-04-03) |
| Crunchbase Organization Id | north-carolina-a-t-state-university |
| Count Of Students | 12,753 (as of 2020-09), 12,387 (as of 2021-09-01) |
| Employees | 1,993 (as of 2020-09) |
| Admission Rate | 0 (as of 2020) |
| Admission Yield Rate | 0 (as of 2020) |
| Endowment | 164,541,130 United States dollar (as of 2022-06-30), 157,335,511 United States dollar (as of 2021-06-30) |
| Academic Calendar Type | semester |
| Sevis School Id | 7095 |
| Tclf Id | north-carolina-agricultural-technical-state-university |
| Kisti Id | K000207616 |
| Domain Name | ncat.edu |
| ‎Yale Lux Id | group/61796380-a30c-4399-921f-de3550c09c0d |
| Encyclopedia Of African American History 1896 To The Present Entry Id | 0917 |
| Threads Username | ncatsuaggies |
| Athletics Program | North Carolina A&T Aggies |
| Motto Text | Mens et Manus |
| Language Used | American English |
