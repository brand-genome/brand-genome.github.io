---
title: Jet
slug: jet
description: ""
date: "2026-03-05T22:38:39.707087"
draft: false
sectors:
  - "e-commerce"
regions:
  - usa
years:
  - 2019
languages:
  - english
tags:
  - internet
industries: []
countries: []
revenue_buckets: []
operating_income_buckets: []
net_profit_buckets: []
lightings: []
perspectives: []
image_backgrounds: []
color_schemes: []
photography_genres: []
concepts: []
depths: []
image_effects: []
dominant_colors: []
color_tones: []
hair_styles: []
facial_expressions: []
clothing_styles: []
clothing_colors: []
posings: []
gazes: []
body_sections: []
logo_elements:
  - primary
  - secondary
brand_colors:
  - purple
typographies: []
imagery_styles:
  - bold
  - photography
  - candid
  - lifestyle
wikidata_description: Australian rock band
has_twitter: false
has_guidelines: true
promotion_image_count: 0
guideline_count: 1
---
## Official Websites

- [http://www.Jet.com](http://www.Jet.com)
- [https://www.walmart.com/](https://www.walmart.com/)

## Brand Guidelines

### 2019

**Logo Information:**

- Jet logo. Download logos at purple.jet.com. Primary and secondary logo options include ensuring legibility—place secondary logo over a solid Jet purple #360060 color block for better contrast. Spacing: At least 20 px padding around the logo. Minimum size: Digital - 75 pixels height, Print - 0.75 inches height. Placement: Over image or solid colors.

**Color Information:**

- PRIMARY: Purple, SECONDARY FOR BACKGROUND: Neutral Colors. CREATIVE CONTENT COLORS include supplementary, light, bright, warmer, cooler tints. Examples of key Jet Purple: #360060 (C97 M100 Y0 K18, PMS 2745).

**Typography Information:**

- Font: Bloomfield family includes Light, Regular, and Bold weights. Downloadable through purple.jet.com.

**Imagery Photography:**

- Jet photography approach focuses on candid, empathetic, and relatable imagery. Scenes feature a 'lightly lived-in' appearance like toys in a room, magazines on chairs, or ice cream for breakfast, conveying a real-life connection.

**Spacing Layout:**

- EMAIL layouts focus on a mobile-first rule: straight headlines with direct response deals using lifestyle imagery and emojis.

**Brand Voice:**

- Editing for tone includes details like sentence casing for headlines and comprehensive structure (title cases for CTAs). Ensures clarity while maintaining brand relatability.

**Notes:**

- Jet Design System (JDS): UI Components, design principles supporting digital systems.

## Additional Properties

| Property | Value |
|----------|-------|
| Musicbrainz Artist Id | 4efa55ba-93cf-497f-baf3-2ca9da7e193e |
| Location Of Formation | Melbourne |
| Genre | alternative rock |
| Freebase Id | /m/02dzcv |
| Gnd Id | 16021403-8 |
| Munzinger Pop Id | 02000001365 |
| Image | Jet 2025.jpg |
| Allmusic Artist Id | mn0000346806 |
| Discogs Artist Id | 46819 |
| Imdb Id | nm1643063 |
| X (Twitter) Username | JetTheBand (as of 2020-04-25, from 2009-06-01), jettheband |
| Inception | 2001 |
| Country Of Origin | Australia |
| Last.Fm Id | Jet |
| Record Label | Elektra, Atlantic Records |
| Has Part(S) | Nic Cester (from 2001, until 2019), Cameron Muncey (from 2001, until 2019), Chris Cester (from 2001, until 2019), Mark Wilson (from 2002, until 2019) |
| Myspace Id | jet |
| Laut.De Artist Id | Jet |
| Songkick Artist Id | 7331 |
| Muziekweb Performer Id | M00000003876 |
| Billboard Artist Id | jet |
| Nl Cr Aut Id | xx0140896 |
| Different From | Jet, Jet |
| Acharts.Co Artist Id | jet |
| Official Name | Jexcks |
| Start Of Work Period | 2001 |
| Viaf Cluster Id | 150254501, 128892842 |
| Spotify Artist Id | 5ypxebeHEIXjMtJb17uJlI |
| Deezer Artist Id | 597 |
| Apple Music Artist Id | 2502435 |
| Topic'S Main Category | Category:Jet (Australian band) |
| Topic Has Template | Template:Jet |
| Larousse Id | groupe-musical/wd/155050 |
| Social Media Followers | 8,083 (as of 2021-01-08), 8,277 (as of 2020-04-25) |
| Youtube Channel Id | UCDY3bjtG-e456ADVpfDZ4Tw (as of 2021-06-13, from 2013-05-16), UC5vZ92ZTaMqw7lLFDvFNu0A (as of 2021-06-13, from 2006-08-30) |
| Isni | 0000000122384621 |
| Facebook Username | jettheband |
| Instagram Username | jet.theband |
| Tidal Artist Id | 17764 |
| Secondhandsongs Artist Id | 11849 |
| Rate Your Music Artist Id | jet |
| Musik Sammler.De Artist Id | 5166 |
| Bbc Things Id | e67315fd-f856-43ba-a57d-00cbaee0c05b |
| Kinobox Person Id | 596792 |
| Library Of Congress Authority Id | no2003105536 |
| Album Of The Year Artist Id | 7564 |
